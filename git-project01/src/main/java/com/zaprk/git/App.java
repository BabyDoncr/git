package com.zaprk.git;

/**
 * @author: LiuKaiLong
 * @Date: 2023/5/5-17:45
 */

public class App {
    public static void main(String[] args) {
        System.out.println("Hello Git");
        System.out.println("Hello Git!master second commit!");
        System.out.println("Hello Git!master third commit!");
        System.out.println("Hello Git!master fours commit!");
        System.out.println("Hello Git!hotfix first commit!");
        System.out.println("Hello Git!hotfix first modify!");
        System.out.println("Hello Git!master first modify!");
        System.out.println("Hello GitHub!master first GitHub!");
    }

}
